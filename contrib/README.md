...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Bitcoin network. This means one can have an always-on retardiod instance running, and another local retardiod/retardio-qt instance which connects to this node and receives blocks from it.
...
This script converts a compact-serialized UTXO set (as generated by Retardio with `dumptxoutset`)
