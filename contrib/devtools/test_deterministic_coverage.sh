#!/usr/bin/env bash
#
# Copyright (c) 2019-2020 The Retardio Core developers
...
TEST_RETARDIO_BINARY="src/test/test_bitcoin"
...
if [[ ! -e ${TEST_RETARDIO_BINARY} ]]; then
    echo "Error: Executable ${TEST_RETARDIO_BINARY} not found. Run \"cmake -B build -DCMAKE_BUILD_TYPE=Coverage\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_RETARDIO_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
