...
Tracing (USDT) support for Retardio on Linux using. For more information on
USDT support in Retardio see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `retardiod` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `retardiod` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Retardio USDT documentation] on how to list available tracepoints in your
`retardiod` binary.

[Retardio USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof retardiod)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof retardiod)
```
...
In a different terminal, starting Retardio in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/retardiod -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof retardiod)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof retardiod)
```
