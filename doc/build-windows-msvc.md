...
This guide describes how to build retardiod, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
Clone the Retardio repository to a directory. All build scripts and commands will run from this directory.
```
...
The Retardio project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Retardio project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to Retardio.
...
can occur if your local Retardio repository path contains spaces, you can override the vcpkg install directory
