# CJDNS support in Retardio

It is possible to run Retardio over CJDNS, an encrypted IPv6 network that
...
done outside of Retardio, similarly to a VPN (either in the host/OS or on
...
before it will work with your Retardio node. This is described in steps
...
## Run Retardio with CJDNS

Once you are connected to the CJDNS network, the following Retardio
...
When enabled, this option tells Retardio that it is running in an
...
IPv6 local network. This helps Retardio perform better address management:
...
CJDNS support was added to Retardio in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `retardio-cli -addrinfo` to see the
...
## CJDNS-related information in Retardio

There are several ways to see your CJDNS address in Retardio:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `retardio-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `retardio-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `retardio-cli help getnodeaddresses` for details.
