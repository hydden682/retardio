# Support for Output Descriptors in Retardio

Since Retardio v0.17, there is support for Output Descriptors. This is a
...
Retardio v24 extended `wsh()` output descriptor with [Miniscript](https://bitcoin.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25. And since v26 Miniscript expressions can now be used in Taproot descriptors.
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.retardio.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Retardio descriptor wallet to serve as your
...
existing Retardio wallets, a convenience function `combo` is
...
All RPCs in Retardio will include the checksum in their output. Only
