...
The data directory is the default location where the Retardio files are stored.
...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `retardiod` or `retardio-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bitcoin_rw.conf`     | Contains [configuration settings](bitcoin-conf.md) modified by `retardiod` or `retardio-qt`; can be specified by `-confrw` option
`./`               | `retardiod.pid`        | Stores the process ID (PID) of `retardiod` or `retardio-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `retardiod` or `retardio-qt`; can be specified by `-debuglogfile` option
...
`retardio-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
...
These subdirectories and files are no longer used by Retardio:
