# Fuzzing Retardio using libFuzzer

## Quickstart guide

To quickly get started fuzzing Retardio using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
## Overview of Retardio fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of Retardio fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `retardiod` arguments to the `fuzz` executable.
...
an instrumentalized libc++, then using it to build [Retardio dependencies
from source](../depends/README.md) and finally the Retardio fuzz binary
...
Every single pull request submitted against the Retardio repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Retardio more robust.
...
# Fuzzing Retardio using afl++

## Quickstart guide

To quickly get started fuzzing Retardio using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Retardio using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Retardio using [Honggfuzz](https://github.com/google/honggfuzz):
...
Retardio participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoin-core)
...
Retardio follows its [security disclosure policy](https://bitcoincore.org/en/security-advisories/),
