...
This tutorial will describe how to use two instances of Retardio, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
```sh
[offline]$ ./build/bin/retardio-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/bin/retardio-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/retardio-cli -signet -named createwallet \
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/bin/retardio-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/retardio-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/retardio-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/retardio-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/retardio-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/retardio-cli -signet -rpcwallet="watch_only_wallet" listtransactions
