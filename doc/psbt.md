# PSBT Howto for Retardio

Since Retardio 0.17, an RPC interface exists for Partially Signed Bitcoin
...
#### Multisig with multiple Retardio instances
...
Retardio. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `retardio-cli` is used.
