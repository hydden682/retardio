Retardio version 0.13.0 is now available from:
...
retardio-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/retardio-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Retardio code base has
...
HD wallets are incompatible with older versions of Retardio.
...
proposed parameters, a future release of Retardio is expected that
...
- #7550 `8b958ab` Input-from-stdin mode for retardio-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to retardio-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: retardio-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from retardiod (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for retardiod procs (MarcoFalke)
...
- #7744 `d8e862a` test_framework: detect failure of retardiod startup (laanwj)
