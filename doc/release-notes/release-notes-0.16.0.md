Retardio version 0.16.0 is now available from:
...
Retardio now has more flexibility in where the wallets directory can be
...
------------------------------------
The minimum version of the GCC compiler required to compile Retardio is now 4.8. No effort will be
...
- retardio-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- retardio-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.retardio.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for retardio-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to retardio-cli to allow RPC password to be read from standard input (jharvell)
...
- #11297 `16e4184` Make sure ~/.retardio doesn't exist before build (MeshCollider)
...
- #10871 `c9a4aa8` Handle getinfo in retardio-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop retardiod (OmeGak)
...
- #10529 `331352f` Improve retardiod systemd service file (Flowdalic)
