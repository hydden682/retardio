Retardio version 0.17.0 is now available from:
...
be used in V0.17 by starting retardiod with the '-deprecatedrpc=accounts'
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart retardiod with
...
- When running retardiod with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If retardiod
...
- #11962 `0a01843` add seed.retardio.sprovoost.nl to DNS seeds (Sjors)
...
- #13486 `66e1a08` Move rpc/util.cpp from libretardio-util to libbitcoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for retardio-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling retardio-qt (TheCharlatan)
...
- #13687 `9d26b69` travis: Check that ~/.retardio is never created (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in retardiod systemd service file (Flowdalic)
- #12652 `55f490a` retardio-cli: Provide a better error message when retardiod is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to retardiod in debian package (ghost)
...
- #13603 `453ae5e` retardio-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix retardio-cli --version (Empact)
...
- #12877 `2b54155` Use retardiod in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed retardio-cli -help output for help2man (hebasto)
