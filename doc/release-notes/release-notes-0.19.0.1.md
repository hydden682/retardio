Retardio version 0.19.0.1 is now available from:
...
Retardio is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Retardio on unsupported systems.
...
to open the Retardio .dmg. This is due to new signing requirements
imposed by Apple, which the Retardio project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Retardio on systems with
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `retardiod -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Retardio with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Retardio 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Retardio. (#14054)

- To eliminate well-known denial-of-service vectors in Retardio,
  ...
  This prevents Retardio from sending the BIP111 NODE_BLOOM service
  ...
  of Retardio and nodes that have manually enabled BIP37 support,
...
- By default, Retardio will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `retardio-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Retardio determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Retardio would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Retardio will be bound to
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Retardio v0.17.0), this can eliminate a
...
- #15639 retardio-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in retardiod/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of retardiod build (NicolasDorier)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Retardio (luke-jr)
...
- #16489 log: harmonize retardiod logging (jonatack)
...
- #16863 Add a missing closing parenthesis in the retardio-wallet's help (darosior)
...
- #16912 Remove Doxygen intro from src/retardiod.cpp (ch4ot1c)
