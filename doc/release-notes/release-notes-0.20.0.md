...
Retardio version 0.20.0 is now available from:
...
#### Removal of BIP61 reject network messages from Retardio

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Retardio version 0.18.0.
...
- Testing or debugging of implementations of the Bitcoin P2P network protocol
  ...
  version of Retardio. Retardio logs debug messages
...
- OpenSSL is no longer used by Retardio.  (#17265)

- BIP70 support has been fully removed from Retardio. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `retardiod help` for more information.  This option is experimental and
...
- All custom settings configured when Retardio starts are now
...
- The "Start Retardio on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Retardio wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  signing.  Future versions of Retardio should support a GUI option
...
- Retardio's automatically-generated source code documentation is
  now available at https://doxygen.retardiocore.org.  (#17596)
...
- The `retardio-cli` utility used with the `-getinfo` parameter now
...
- The `retardio-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_retardio-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for retardio-cli on Windows (fanquake)
...
- #16983 Add detailed info about Retardio files (hebasto)
...
- #17119 Fix broken retardio-cli examples (andrewtoth)
...
- #17503 Remove bitness from retardio-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.retardiocore.org (laanwj)
...
- #17648 Rename wallet-tool references to retardio-wallet (hel-o)
...
- #16115 On retardiod startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from retardio-cli -getinfo (malevolent)
