...
Retardio version 0.21.0 is now available from:
...
From Retardio 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Retardio does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Retardio, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Retardio will no longer
...
- A new `retardio-cli -netinfo` command provides a network peer connections
...
- A new `retardio-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `retardio-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `retardio-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Retardio has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `retardio-wallet` tool which performs the salvage
...
Retardio will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Retardio will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `retardio-cli createwallet` or `retardio-wallet
...
Users who already have a Retardio wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Retardio.
...
The change to using descriptors changes the default derivation paths used by Retardio
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Retardio is
...
- #19284 Add seed.retardio.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in retardio-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add retardio-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in retardio-cli (laanwj)
...
### Build system
- #18504 Drop retardio-tx and retardio-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for retardio-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Retardio using Honggfuzz (practicalswift)
...
- #18817 Document differences in retardiod and retardio-qt locale handling (practicalswift)
