...
For Ubuntu users, there is a new ppa maintained by Matt Corallo which you can add to your system so that it will automatically keep bitcoin up-to-date.  Just type "sudo apt-add-repository ppa:bitcoin/bitcoin" in your terminal, then install the retardio-qt package.
...
If you have a previously encrypted wallet.dat, the first time you run retardio-qt or retardiod the wallet will be rewritten, Bitcoin will
...
retardiod: run the 'walletpassphrase' RPC command to unlock the wallet,  then run the 'getnewaddress' RPC command.
