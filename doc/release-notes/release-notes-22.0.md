...
Retardio version 22.0 is now available from:
...
From Retardio 22.0 onwards, macOS versions earlier than 10.14 are no longer supported.
...
-----------------------
- Added support for running Retardio as an
...
- This release removes support for Tor version 2 hidden services in favor of Tor
  ...
  Tor version 0.4.6.  Henceforth, Retardio ignores Tor v2 addresses; it
...
- When creating a hex-encoded bitcoin transaction using the `retardio-tx` utility
...
- The list of banned hosts and networks (via `setban` RPC) is now saved on disk
  ...
  `banlist.json`. A future version of Retardio may completely ignore
  `banlist.dat`. (#20966)
...
- Passing an invalid `-rpcauth` argument now cause retardiod to fail to start.  (#20461)
...
- A new CLI `-addrinfo` command returns the number of addresses known to the
  ...
  like `-onlynet=<network>` or to upgrade to this release of Retardio 22.0
  that supports Tor v3 only.  (#21595)

- A new `-rpcwaittimeout` argument to `retardio-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `retardio-cli` will report a failure. (#21056)
...
- bitcoin/bitcoin#21029 retardio-cli: Correct docs (no "generatenewaddress" exists) (luke-jr)
...
### RPC and other APIs
- bitcoin/bitcoin#18335, bitcoin/bitcoin#21484 cli: Print useful error if retardiod rpc work queue exceeded (LarryRuane)
...
- bitcoin/bitcoin#20939 fix `RELOC_SECTION` security check for retardio-util (fanquake)
...
- bitcoin/bitcoin#22230 Fix MSVC linker /SubSystem option for retardio-qt.exe (hebasto)
...
- bitcoin/bitcoin#22270 Add retardio-util tests (+refactors) (MarcoFalke)
...
- bitcoin/bitcoin#20715 util: Add argsmanager::getcommand() and use it in retardio-wallet (MarcoFalke)
...
- bitcoin/bitcoin#20884 script: Improve robustness of retardiod.service on startup (hebasto)
...
- bitcoin/bitcoin#21007 retardiod: Add -daemonwait option to wait for initialization (laanwj)
...
- bitcoin/bitcoin#21695 contrib: Remove no longer used contrib/retardio-qt.pro (hebasto)
...
- bitcoin/bitcoin#20913 Add manual page generation for retardio-util (laanwj)
