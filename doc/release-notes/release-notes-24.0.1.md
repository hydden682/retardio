...
Retardio version 24.0.1 is now available from:
...
This version of Retardio adds a new `mempoolfullrbf` configuration
...
fees, was realized in [Retardio 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Retardio are
...
[Retardio 0.12.0]: https://bitcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- To help prevent fingerprinting transactions created by the Retardio wallet, change output
...
config files), so these settings will now apply to retardiod, instead of being
