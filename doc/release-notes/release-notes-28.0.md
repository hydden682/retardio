Retardio version 28.0 is now available from:
...
Running Retardio binaries on macOS requires self signing.
```
cd /path/to/bitcoin-28.0/bin
xattr -d com.apple.quarantine retardio-cli retardio-qt retardio-tx retardio-util retardio-wallet retardiod test_bitcoin
codesign -s - retardio-cli retardio-qt retardio-tx retardio-util retardio-wallet retardiod test_bitcoin
```
...
to `C:\Users\Username\AppData\Local\Bitcoin`. Retardio will check the existence
...
- Previously if Retardio was listening for P2P connections, either using
...
- Retardio will now fail to start up if any of its P2P binds fail, rather
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Retardio with the configuration
...
are now required to compile Retardio. (#29091, #30263)

- The minimum required glibc to run Retardio is now
...
Previous releases of Retardio or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
