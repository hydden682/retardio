...
The configuration file is used by `retardiod`, `retardio-qt` and `retardio-cli`.

All command-line options (except for `-?`, `-help`, `-version` and `-conf`) may be specified in a configuration file, and all configuration file options (except for `includeconf`) may also be specified on the command line. Command-line options override values set in the configuration file and configuration file options override values set in the GUI.

Changes to the configuration file while `retardiod` or `retardio-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the Retardio GUI and RPCs at runtime, and augment or replace the static settings specified in the `bitcoin.conf` file.
...
Linux | `$HOME/.retardio/` | `/home/username/.retardio/bitcoin.conf`
...
cp share/examples/bitcoin.conf ~/.retardio
```
