#!/usr/bin/env python3
"""
Retardio Stratum Pool v7 - Fixed for NerdMiner
Based on NerdMiner source analysis (issue #513):
- Version: reversed
...
def merkle_root(coinbase_hash, branches):
    """Calculate merkle root - NerdMiner doesn't reverse branches"""
    current = coinbase_hash
    for branch in branches:
        # NerdMiner uses branches directly without reversal
...
class StratumMiner:
    ...
    async def send_job(self, clean=True):
        ...
        # Prevhash: word-swap for stratum format
        ...
        # Version: send as LE hex (NerdMiner will reverse it)
        ...
        print(f"[>] Job {job_id} (height={height})")
    ...
    async def handle_submit(self, msg_id, params):
        ...
        # === MERKLE ROOT (not reversed by NerdMiner) ===
        mr = merkle_root(coinbase_hash, job["merkle_branches"])

        # === BUILD HEADER EXACTLY AS NERDMINER DOES ===
        # NerdMiner receives hex strings and reverses: version, prevhash(word), ntime, nbits
        # Merkle root is NOT reversed

        # Version: NerdMiner reverses the hex bytes
        version_bytes = reverse_bytes(bytes.fromhex(job["version_hex"]))

        # Prevhash: NerdMiner word-swaps the stratum format
        prevhash_bytes = swap_endian_words(job["prevhash_stratum"])

        # Merkle: NOT reversed (used as-is from sha256d)
        merkle_bytes = mr

        # ntime: NerdMiner reverses the hex bytes
        ntime_bytes = reverse_bytes(bytes.fromhex(ntime_hex))

        # nbits: NerdMiner reverses the hex bytes
        nbits_bytes = reverse_bytes(bytes.fromhex(job["nbits"]))

        # nonce: NerdMiner finds this during mining, sends back reversed
        # We need to reverse it back to get what NerdMiner used in the header
        ...
        print(f"    nonce: {nonce_hex} -> {nonce_bytes.hex()}")
...
async def main():
    print("=" * 60)
    print("  RETARDIO POOL v7 - NerdMiner Compatible")
    print("=" * 60)
