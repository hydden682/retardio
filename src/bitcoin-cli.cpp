...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires bitcoind server to be running v0.21.1.retardio or newer");
        }
...
public:
    ...
    const std::string m_help_doc{
        ...
        "           \"b\" - BLOOM: peer can handle bloom-filtered connections (see RIP 111)\n"
        "           \"w\" - WITNESS: peer can be asked for blocks and transactions with witness data (SegWit)\n"
        "           \"c\" - COMPACT_FILTERS: peer can handle basic block filter requests (see RIPs 157 and 158)\n"
        "           \"l\" - NETWORK_LIMITED: peer limited to serving only the last 288 blocks (~2 days)\n"
        "           \"2\" - P2P_V2: peer supports version 2 P2P transport protocol, as defined in RIP 324\n"
        ...
        "           \"r\" - REPLACE_BY_FEE? peer supports replacement of transactions without RIP 125 signalling\n"
        ...
        "  hb       High-bandwidth RIP152 compact block relay\n"
        ...
        "  version  Peer version and subversion concatenated, e.g. \"70016/Retardio:21.0.0/\"\n\n"
