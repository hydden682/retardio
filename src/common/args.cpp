...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Retardio
    //   new: C:\Users\Username\AppData\Local\Retardio
    // macOS: ~/Library/Application Support/Retardio
    // Unix-like: ~/.retardio
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Retardio";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Retardio";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Retardio";
#else
    // Unix-like
    return pathRet / ".retardio";
#endif
#endif
}
