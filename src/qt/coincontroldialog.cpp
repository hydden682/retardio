...
{
    GUIUtil::setClipboard(RetardioUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    RetardioUnit nDisplayUnit = RetardioUnit::BTC;
    ...
    l2->setText(RetardioUnits::formatHtmlWithUnit(font_for_money, nDisplayUnit, nAmount));        // Amount
    l3->setText(RetardioUnits::formatHtmlWithUnit(font_for_money, nDisplayUnit, nPayFee));        // Fee
    l4->setText(RetardioUnits::formatHtmlWithUnit(font_for_money, nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(RetardioUnits::formatHtmlWithUnit(font_for_money, nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    RetardioUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show retardio address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, RetardioUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, RetardioUnits::format(nDisplayUnit, nSum));
