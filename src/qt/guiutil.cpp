...
QFont fixedPitchFont(bool use_embedded_font)
{
    if (use_embedded_font) {
        ...
        return {"OCR-Retardio", QFont().pointSize()};
    }
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Retardio address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new RetardioAddressEntryValidator(parent));
...
{
    // return if URI is not valid or is no retardio: URI
    if(!uri.isValid() || uri.scheme() != QString("retardio"))
        return false;
...
QString formatRetardioURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("retardio:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(RetardioUnits::format(RetardioUnit::BTC, info.amount, false, RetardioUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinConf()
{
    ...
    /* Open retardio.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Retardio.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Retardio (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Retardio (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Retardio*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "retardio.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("retardio-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a retardio.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Retardio\n";
        else
            optionFile << strprintf("Name=Retardio (%s)\n", ChainTypeToString(chain));
