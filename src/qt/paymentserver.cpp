...
const QString BITCOIN_IPC_PREFIX("retardio:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // retardio: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click retardio: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start retardio: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling retardio: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("retardio://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'retardio://' is not a valid URI. Use 'retardio:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // retardio: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Retardio address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
