...
void PSBTOperationsDialog::saveTransaction() {
    ...
    const RetardioUnits::Unit unit = m_client_model->getOptionsModel()->getDisplayUnit();
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = RetardioUnits::format(unit, out.nValue, /*plussign=*/false, RetardioUnits::SeparatorStyle::NEVER);
        if (unit != RetardioUnits::Unit::BTC) amount += RetardioUnits::shortName(unit);  // NOTE: no space
...
{
    const QFont font_for_money_BTC = m_client_model->getOptionsModel()->getFontForMoney(RetardioUnit::BTC);
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(bullet_point).append(tr("Sends %1 to %2")
            .arg(RetardioUnits::formatHtmlWithUnit(font_for_money_BTC, RetardioUnit::BTC, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(RetardioUnits::formatHtmlWithUnit(font_for_money_BTC, RetardioUnit::BTC, *analysis.fee));

        // add total amount in all subdivision units
        tx_description.append("<hr />");
        QStringList alternativeUnits;
        for (const RetardioUnits::Unit u : RetardioUnits::availableUnits())
        {
        ...
        const RetardioUnit display_unit = m_client_model->getOptionsModel()->getDisplayUnit();
