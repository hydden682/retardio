...
const std::string RetardioGUI::DEFAULT_UIPLATFORM =
...
RetardioGUI::RetardioGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &RetardioGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &RetardioGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &RetardioGUI::showModalOverlay);
...
void RetardioGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Retardio address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and retardio: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &RetardioGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &RetardioGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &RetardioGUI::gotoHistoryPage);
    connect(m_action_pairing, &QAction::triggered, this, [this]{ showNormalIfMinimized(); });
    connect(m_action_pairing, &QAction::triggered, this, &RetardioGUI::gotoPairingPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Retardio addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Retardio addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Retardio Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Retardio Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a retardio: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Retardio command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &RetardioGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &RetardioGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &RetardioGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &RetardioGUI::showHelpMessageClicked);
    connect(m_show_netwatch_action, &QAction::triggered, this, &RetardioGUI::showNetWatch);
    connect(openRPCConsoleAction, &QAction::triggered, this, &RetardioGUI::showDebugWindow);
    connect(showMempoolStatsAction, &QAction::triggered, this, &RetardioGUI::showMempoolStatsWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &RetardioGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &RetardioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &RetardioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &RetardioGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &RetardioGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &RetardioGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &RetardioGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &RetardioGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &RetardioGUI::showDebugWindow);
}

void RetardioGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void RetardioGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &RetardioGUI::setCurrentWalletBySelectorIndex);
...
void RetardioGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &RetardioGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &RetardioGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &RetardioGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &RetardioGUI::showProgress);
...
#ifdef ENABLE_WALLET
void RetardioGUI::enableHistoryAction(bool privacy)
{
...
void RetardioGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &RetardioGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &RetardioGUI::removeWallet);
...
WalletController* RetardioGUI::getWalletController()
{
    return m_wallet_controller;
}

void RetardioGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &RetardioGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &RetardioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &RetardioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &RetardioGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &RetardioGUI::incomingTransaction);
    connect(this, &RetardioGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void RetardioGUI::removeWallet(WalletModel* walletModel)
{
...
void RetardioGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void RetardioGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void RetardioGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void RetardioGUI::setWalletActionsEnabled(bool enabled)
{
...
void RetardioGUI::createTrayIcon()
{
...
void RetardioGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &RetardioGUI::toggleHidden);
...
void RetardioGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void RetardioGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void RetardioGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void RetardioGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void RetardioGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

void RetardioGUI::showMempoolStatsWindow()
{
...
void RetardioGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RetardioGUI::gotoPairingPage()
{
    m_action_pairing->setChecked(true);
    if (walletFrame) walletFrame->gotoPairingPage();
}

void RetardioGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RetardioGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RetardioGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RetardioGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RetardioGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void RetardioGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void RetardioGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Retardio network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void RetardioGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RetardioGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void RetardioGUI::updateHeadersSyncProgressLabel()
{
...
void RetardioGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void RetardioGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &RetardioGUI::quitRequested);
...
void RetardioGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void RetardioGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &RetardioGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void RetardioGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void RetardioGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &RetardioGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &RetardioGUI::show);
                e->ignore();
            }
        }
    }
...
void RetardioGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RetardioGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RetardioGUI::incomingTransaction(const QString& date, RetardioUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(RetardioUnits::formatWithUnit(unit, amount, true));
...
void RetardioGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RetardioGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RetardioGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RetardioGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RetardioGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void RetardioGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void RetardioGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void RetardioGUI::updateProxyIcon()
{
    std::string ip_port;
...
void RetardioGUI::updateWindowTitle()
{
...
void RetardioGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RetardioGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RetardioGUI::detectShutdown()
{
...
void RetardioGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void RetardioGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RetardioGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = RetardioGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void RetardioGUI::subscribeToCoreSignals()
{
...
void RetardioGUI::unsubscribeFromCoreSignals()
{
...
bool RetardioGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<RetardioUnit> units = RetardioUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RetardioUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, RetardioUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const RetardioUnit u : RetardioUnits::availableUnits()) {
        menu->addAction(RetardioUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(RetardioUnit newUnits)
{
    setText(RetardioUnits::longName(newUnits));
}
