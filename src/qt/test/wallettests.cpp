...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_retardio-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_retardio-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_retardio-qt  # macOS
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_retardio-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
