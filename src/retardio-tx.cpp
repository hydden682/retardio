...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " retardio-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The retardio-tx tool is used for creating and modifying retardio transactions.\n\n"
                "retardio-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded retardio transaction, or with \"-create [commands]\" to create a hex-encoded retardio transaction.\n"
                "\n"
                "Usage: retardio-tx [options] <hex-tx> [commands]\n"
                "or:    retardio-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded retardio transaction
